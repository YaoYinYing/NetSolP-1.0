...
def get_preds_split(split_i, embed_dataloader, args, prediction_type, test_df):
    ...
    if DEVICE == 'CPU':
        ort_sessions = [onnxruntime.InferenceSession(mp, sess_options=opts,providers=['CPUExecutionProvider']) for mp in model_paths]
    else:
        ort_sessions = [onnxruntime.InferenceSession(mp, sess_options=opts, providers=['CUDAExecutionProvider']) for mp
                        in model_paths]
    embed_dict = {}
...
def run_model_distilled(embed_dataloader, args, prediction_type, test_df):
    ...
    if DEVICE == 'CPU':
        ort_sessions = [onnxruntime.InferenceSession(mp, sess_options=opts, providers=['CPUExecutionProvider']) for mp
                        in model_paths]
    else:
        ort_sessions = [onnxruntime.InferenceSession(mp, sess_options=opts, providers=['CUDAExecutionProvider']) for mp
                        in model_paths]
